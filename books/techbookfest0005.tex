...
\begin{proof}%
  ...
  \item $length(\Pi_1^{})=1$ の場合．%
    ...
    \end{equation}%
    という形をしている．この場合は$\mathit{e}_3^{}$が存在するのでよい．%
  \item $length(\Pi_2^{})=1$ の場合．%
    $length(\Pi_1^{})=1$ の場合とまったく同様．%
  \item $length(\Pi_1^{})=k+1$,$length(\Pi_2^{})=l+1$ の場合．%
    $\Pi_1^{}$ は%
    \begin{equation}%
      \infer{%
        \mathit{e}_1^{}\sqsupset\mathit{e}_2^{}%
    ...
    という形をしている．帰納法の仮定:%
    \begin{align}%
      \forall\mathit{e}_3^{}\exists\mathit{e}_4^{}%
      ((\vdash\mathit{e}_5^{}\sqsupset\mathit{e}_2^{})\rightarrow%
      (\vdash\mathit{e}_5^{}\sqsupset\mathit{e}_3^{})\rightarrow%
      (\vdash\mathit{e}_2^{}\sqsupset\mathit{e}_4^{})\land%
      (\vdash\mathit{e}_3^{}\sqsupset\mathit{e}_4^{}))\\%
      \forall\mathit{e}_2^{}\exists\mathit{e}_4^{}%
      ((\vdash\mathit{e}_6^{}\sqsupset\mathit{e}_2^{})\rightarrow%
      (\vdash\mathit{e}_6^{}\sqsupset\mathit{e}_3^{})\rightarrow%
      (\vdash\mathit{e}_2^{}\sqsupset\mathit{e}_4^{})\land%
      (\vdash\mathit{e}_3^{}\sqsupset\mathit{e}_4^{}))%
    \end{align}%
    ...
    $\mathit{e}_5^{}\sqsupset_1^{}\mathit{e}_7^{}$だから%
    帰納法の仮定により%
    ...
    $\mathit{e}_6^{}\sqsupset\mathit{e}_8^{}$である．%
    $\mathit{e}_6^{}\sqsupset\mathit{e}_3^{}$かつ%
    $\mathit{e}_6^{}\sqsupset_1^{}\mathit{e}_8^{}$だから%
    帰納法の仮定により%
    $\mathit{e}_3^{}\sqsupset\mathit{e}_9^{}$かつ%
    $\mathit{e}_8^{}\sqsupset\mathit{e}_9^{}$となるような%
    $\mathit{e}_9^{}$が存在する．%
    $\mathit{e}_2^{}\sqsupset\mathit{e}_8^{}$かつ%
    $\mathit{e}_8^{}\sqsupset\mathit{e}_9^{}$だから%
    \lemmaname\ref{lemma:par_reduct_trans}により%
    $\mathit{e}_2^{}\sqsupset\mathit{e}_9^{}$．%
    ゆえに$\mathit{e}_2^{}\sqsupset\mathit{e}_9^{}$かつ%
    $\mathit{e}_3^{}\sqsupset\mathit{e}_9^{}$となるような%
    $\mathit{e}_9^{}$が存在する．%
  \end{itemize}%
\end{proof}%
