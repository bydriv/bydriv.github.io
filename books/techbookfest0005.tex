...
\begin{lemma}%
  ...
  さらに$length(\vdash\mathit{e}_1^{}\sqsupset\mathit{e}_3^{})=length(\vdash\mathit{e}_1^{}\sqsupset\mathit{e}_2^{})+length(\vdash\mathit{e}_2^{}\sqsupset\mathit{e}_3^{})-1$．%
\end{lemma}%
\begin{proof}%
  ...
  \item $length(\Pi)=1$ の場合．%
    ...
    \begin{equation}%
      \infer{\mathit{e}_1^{}\sqsupset\mathit{e}_3^{}}{\Pi_1^{}&\cdots&\Pi_n^{}}%
    \end{equation}%
    と変形できるからよい．長さに関しても$\vdash\mathit{e}_1^{}\sqsupset\mathit{e}_1^{}$を%
    はぶいているので$1$減っている．%
  \item $length(\Pi)=k + 1$ の場合．%
    $\mathit{e}_1^{}\sqsupset\mathit{e}_3^{}$に至る証明図は%
    \begin{equation}%
    ...
    (\vdash\mathit{e}_2^{}\sqsupset\mathit{e}_3^{})\rightarrow%
    (\vdash\mathit{e}_4^{}\sqsupset\mathit{e}_3^{})\land%
    length(\vdash\mathit{e}_4^{}\sqsupset\mathit{e}_3^{})=%
    length(\vdash\mathit{e}_4^{}\sqsupset\mathit{e}_2^{})+%
    length(\vdash\mathit{e}_2^{}\sqsupset\mathit{e}_3^{})-1$だから，%
    $\mathit{e}_1^{}\sqsupset\mathit{e}_3^{}$に至る証明図は%
    \begin{equation}%
      \infer{%
        \mathit{e}_1^{}\sqsupset\mathit{e}_3^{}%
    ...
    \end{equation}%
    と変形できるからよい．長さに関しても%
    $\mathit{e}_4^{}\sqsupset\mathit{e}_3^{}$に至る証明図の長さが%
    $1$減っているので，全体としても$1$減っている．%
  \end{itemize}%
\end{proof}%
\section{合流性定理}%
\label{sect:cr}%
