...
\begin{proof}%
  ...
  \item $length(\Pi_1^{})=k+1$, $length(\Pi_2^{})=j+1$ の場合．%
    ...
    $\mathit{e}_6^{}\sqsupset\mathit{e}_3^{}$とする．%
    \lemmaname\ref{lemma:parcr1}により%
    $\mathit{e}_5^{}\sqsupset_1^{}\mathit{e}_7^{}$かつ%
    $\mathit{e}_6^{}\sqsupset_1^{}\mathit{e}_7^{}$となるような%
    $\mathit{e}_7^{}$が存在する．%
    ゆえに\lemmaname\ref{lemma:par_reduct1n}により%
    $\mathit{e}_5^{}\sqsupset\mathit{e}_7^{}$かつ%
    $\mathit{e}_6^{}\sqsupset\mathit{e}_7^{}$である．%
    帰納法の仮定より%
    $\mathit{e}_5^{}\sqsupset\mathit{e}_2^{}$かつ%
    $\mathit{e}_5^{}\sqsupset\mathit{e}_7^{}$ならば%
    $\mathit{e}_2^{}\sqsupset\mathit{e}_8^{}$かつ%
    $\mathit{e}_7^{}\sqsupset\mathit{e}_8^{}$となるような%
    $\mathit{e}_8^{}$が存在し，%
    $\mathit{e}_6^{}\sqsupset\mathit{e}_3^{}$かつ%
    $\mathit{e}_6^{}\sqsupset\mathit{e}_7^{}$ならば%
    $\mathit{e}_3^{}\sqsupset\mathit{e}_9^{}$かつ%
    $\mathit{e}_7^{}\sqsupset\mathit{e}_9^{}$となるような%
    $\mathit{e}_9^{}$が存在する．%
    さらに$\mathit{e}_7^{}\sqsupset\mathit{e}_8^{}$かつ%
    $\mathit{e}_7^{}\sqsupset\mathit{e}_9^{}$だから%
    $\mathit{e}_8^{}\sqsupset\mathit{e}_{10}^{}$かつ%
    $\mathit{e}_9^{}\sqsupset\mathit{e}_{10}^{}$となるような%
    $\mathit{e}_{10}^{}$が存在する．%
    $\mathit{e}_2^{}\sqsupset\mathit{e}_8^{}$かつ%
    $\mathit{e}_8^{}\sqsupset\mathit{e}_{10}^{}$だから%
    \lemmaname\ref{lemma:par_reduct_trans}により%
    $\mathit{e}_2^{}\sqsupset\mathit{e}_{10}^{}$であり，%
    $\mathit{e}_3^{}\sqsupset\mathit{e}_9^{}$かつ%
    $\mathit{e}_9^{}\sqsupset\mathit{e}_{10}^{}$だから%
    同様に%
    $\mathit{e}_3^{}\sqsupset\mathit{e}_{10}^{}$である．%
    ゆえに%
    $\mathit{e}_2^{}\sqsupset\mathit{e}_{10}^{}$かつ%
    $\mathit{e}_3^{}\sqsupset\mathit{e}_{10}^{}$となるような．%
    $\mathit{e}_{10}^{}$が存在する．これが証明すべきことであった．%
  \end{itemize}%
\end{proof}%
\par\lemmaname\ref{lemma:parcr}で%
nステップの並行簡約の合流性を示しました．%
肝心の$\beta$-変形の合流性の証明は，%
nステップの$\beta$-変形の体系がnステップの並行簡約と同値であることを示すことでおこないます．%
...
\begin{lemma}%
  \label{lemma:betapar}%
  すべてのnステップの$\beta$-変形の証明図$\mathit{e}_1^{}\triangleright\mathit{e}_2^{}$には，%
  それと等価なnステップの並行簡約の証明図$\mathit{e}_1^{}\sqsupset\mathit{e}_2^{}$が存在する．%
\end{lemma}%
\begin{proof}
  $\mathit{e}_1^{}\triangleright\mathit{e}_2^{}$に至る証明図を$\Pi$とし，%
  $\Pi$の長さに関する帰納法で証明する．%
  ...
  \item $length(\Pi)=1$ の場合．%
    ...
    \end{equation}%
    という形をしているから，これと等価なnステップの並行簡約の推論規則は%
    \begin{equation}%
      \infer{%
        \mathit{e}_1^{}\sqsupset\mathit{e}_1^{}%
      }{%
  ...
  \item $length(\Pi)=k + 1$ の場合．%
    ...
    \end{equation}%
    という形をしている．帰納法の仮定により%
    $\mathit{e}_2^{}\triangleright\mathit{e}_3^{}$に至る推論図と等価なnステップの%
...
\begin{lemma}%
  \label{lemma:parbeta}%
  すべてのnステップの並行簡約の証明図$\mathit{e}_1^{}\sqsupset\mathit{e}_2^{}$には，%
  それと等価なnステップの$\beta$-変形の証明図$\mathit{e}_1^{}\triangleright\mathit{e}_2^{}$が存在する．%
\end{lemma}%
\begin{proof}
  $\mathit{e}_1^{}\sqsupset\mathit{e}_2^{}$に至る証明図を$\Pi$とし，%
  ...
  \item $length(\Pi)=k + 1$ の場合．%
    ...
    \end{equation}%
    という形をしている．帰納法の仮定により%
    $\mathit{e}_2^{}\sqsupset\mathit{e}_3^{}$に至る推論図と等価なnステップの%
    $\beta$-変形の推論図が存在するし，\lemmaname\ref{lemma:par1beta}により%
    $\mathit{e}_1^{}\sqsupset_1^{}\mathit{e}_2^{}$に至る推論図と等価なnステップの%
    $\beta$-変形の推論図が存在する．さらに\lemmaname\ref{lemma:beta_reduct_trans}により%
    nステップの$\beta$-変形では推移律が成りたつから，これと等価な推論規則は%
    \begin{equation}%
      \infer{%
        \mathit{e}_1^{}\triangleright\mathit{e}_3^{}%
      }{%
...
nステップの$\beta$-変形とnステップの並行簡約は同値である．%
\end{lemma}%
\begin{proof}%
